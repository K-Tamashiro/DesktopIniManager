    ...
    internal static class MftDifferencerService
    {
        ...
        public static List<string> Synchronize(DiffSnapshot snapshot, IEnumerable<DiffFile> selected, bool toTarget, Action<string> onLog = null)
        {
            ...
            foreach (DiffFile file in selected)
            {
                ...
                try
                {
                    string left = SafePath(snapshot.SourceRoot, file.RelativePath), right = SafePath(snapshot.TargetRoot, file.RelativePath);
                    ...
                    string from = toTarget ? left : right, to = toTarget ? right : left;
        ...
        private static bool IsLockedOrAccessDenied(Exception ex)
        {
